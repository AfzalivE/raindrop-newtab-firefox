...
{
  try{ga('send', 'event', 'import', 'open');}catch(e){}
...
//chrome-extension://pcphonafajmhkbhfjjpcanpappafpmmn/import-build/index.html
config = {
  sources: ["chrome", "firefox", "safari", "opera", "delicious", "diigo", "dragdis", "dropmark", "instapaper", "kippt", "pinboard", "pocket", "readability", "file"],
  help: {
    pocket: "http://help.getpocket.com/customer/portal/articles/482643-how-can-i-back-up-or-export-my-list-",
    instapaper: "https://www.instapaper.com/user/export",
    readability: "https://www.readability.com/account/export",
    kippt: "https://kippt.com/extras/export",
    delicious: "http://export.delicious.com/settings/bookmarks/export",
    diigo: "https://www.diigo.com/tools/export",
    pinboard: "https://www.diigo.com/tools/export",
    dragdis: "https://dragdis.com/Account/Settings/export",
    dropmark: "https://tools.dropmark.com/export",
    chrome: "https://support.google.com/chrome/answer/96816",
    firefox: "https://support.mozilla.org/en-US/kb/export-firefox-bookmarks-to-backup-or-transfer",
    safari: "https://support.apple.com/kb/PH17154",
    opera: "https://addons.opera.com/en/extensions/details/bookmarks-import-export"
  }
}
...
window.addEventListener((window.languageLoaded ? "DOMContentLoaded" : "langLoaded"), function() {
  var HomeRoute = require("./routes/Home"),
      ...
      UploadCollectionsRoute = require("./routes/UploadCollections"),
      UploadBookmarksRoute = require("./routes/UploadBookmarks"),
      FinalRoute = require("./routes/Final"),
      ResetRoute = require("./routes/Reset");
...
},{"../actions/User":7,"../app/components/Helpers/Icon.js":8,"../app/config":9,"../modules/api.js":17,"../modules/network.js":18,"../modules/translate":19,"../newtab/manifest.json":20,"../stores/User":23,"./routes/File":11,"./routes/Final":12,"./routes/Home":13,"./routes/Reset":14,"./routes/UploadBookmarks":15,"./routes/UploadCollections":16,"lodash":"lodash","moment":"moment","raven-js":"raven-js","react":"react","react-router":"react-router","reflux":"reflux","string":"string"}],11:[function(require,module,exports){
/** @jsx React.DOM */
var $ = require("jquery"),
...
var _prepareHTML = function(evt, callback, encoding) {
	if (typeof encoding == 'undefined')
		encoding = null;

	var f = false;

  try{f = evt.files[0];}catch(e){}
			...
			if (encoding==null){
				...
				if (eMatches.length>=3){
					...
					if ((eCharset=='utf-8')||(eCharset==''))
						...
						_prepareHTML(evt, callback, eCharset)
				}
				else
					goToCallback()
			}
...
module.exports = React.createClass({
  ...
  componentWillMount: function() {
    var _params = this.context.router.getCurrentParams();
    source = _params.source;
  },

  handleFile: function(e) {
    var _this = this;
    _prepareHTML(e.target, function(contents){
      if (contents!=null){
        queue.reset();
        ...
        switch(source){
          case "readability":
            ...
            if (original!=null){
              ...
              queue.items = items;
              delete original;
              _this.context.router.transitionTo('uploadCollections', {source:source});
            }else{
              alert("Invalid file!");
            }
          break;
          ...
          default:
            ...
            $(prefs.links, htmlFile).each(function(){
              ...
              section = section || (source.charAt(0).toUpperCase() + source.slice(1));
...
},{"jquery":1,"validator":"validator"}],12:[function(require,module,exports){
/** @jsx React.DOM */
var vivus = require("vivus");

module.exports = React.createClass({
  displayName: "routes/Final",
  ...
  getInitialState: function() {
    ...
    return {
      email: email || t.s("specifyYourEmail")
    }
  },

  componentDidMount: function() {
    var img = new vivus('file-img', {
      type: 'async',
      duration: 150,
    ...
    var img2 = new vivus('minifiles', {
      type: 'oneByOne',
      duration: 300,
      pathTimingFunction: vivus.EASE_IN,
      animTimingFunction: vivus.EASE_OUT,
      file: 'images/minifiles.svg'
    }, function() {
      img2
        .stop()
        .reset()
...
},{"vivus":"vivus"}],13:[function(require,module,exports){
/** @jsx React.DOM */
module.exports = React.createClass({
  displayName: "routes/Home",
  ...
  getInitialState: function() {
    return {
      logged: UserStore.isLogged()
    }
  },

  onUserChange: function(parents) {
      if (this.isMounted())
      this.setState({
          logged: UserStore.isLogged()
      });
  ...
  render: function() {
    switch(this.state.logged){
      case true:
        var sources = config.sources.map(function(item){
          var title = item;
          if (title == "file")
            title = "HTML File";
      ...
      case false:
        return (React.createElement("div", {className: "window"}, t.s("startToSave")));
      break;
    }
  }
});
    ...
    var worker = {
      ...
      check: function() {
        ...
        if (worker.index>=worker.count){
          ...
          var nextProgress = parseInt((100/worker.count)*(worker.index+1));
          if (component.state.progress != nextProgress)
            component.setState({progress: nextProgress});
...
module.exports = React.createClass({
  displayName: "routes/UploadBookmarks",
      ...
      var worker = {
        ...
        check: function() {
          ...
          if (this.index>=this.count+1){
            component.context.router.transitionTo('final');
          }else{
            var nextProgress = parseInt((100/this.count)*(this.index));
            if (component.state.progress != nextProgress)
              component.setState({progress: nextProgress});
        ...
        next: function() {
          var _this = this;
          var items = [], urls = [];
          var fromIndex = (this.index*_perQueue);
          for(var i = fromIndex; i < (fromIndex+_perQueue); i++)
            if (typeof queue.bookmarks[i] != "undefined"){
              items.push(queue.bookmarks[i]);
              urls.push(queue.bookmarks[i].link);
            }
            else
              break;

          if (items.length>0){
            Api.post("import/check/urls", {urls: urls}, function(json) {
              if (json.result){
                if (json.duplicates.length>0){
                  var needRemoveIndexes = [];
                  for(var i in items)
                    for(var j in json.duplicates)
                      if (items[i].link == json.duplicates[j].link){
                        needRemoveIndexes.push(i);
                      }
                  needRemoveIndexes = _.uniq(needRemoveIndexes);

                  if (needRemoveIndexes.length!=items.length)
                    _.pullAt(items, needRemoveIndexes);
                  else
                    items = [];
                }

                if (items.length>0)
                  Api.post("import/insert", {items: items, weight: queue.count}, function(result){
                    worker.check();
                  });
                else
                  worker.check();
...
module.exports = React.createClass({
  displayName: "routes/UploadCollections",
  ...
  componentDidMount: function() {
    ...
    if (queue.count==0)
      ...
      var worker = {
        ...
        count: 0,
        index: 0,
        check: function(cId, already) {
          this.items[this.index]._id = cId;
          this.items[this.index].already = already;
          this.items[this.index].items.forEach(function(bookmark){
            bookmark.collectionId = cId;
          ...
          if (this.index>=this.count){
            ...
            for(var i in this.items)
              if (!this.items[i].already)
                cList.push(parseInt(this.items[i]._id));

            UserStore.removeEmptyGroups(function(){
              if (cList.length>0)
                UserStore.onInsertGroup({
                  item: {
                    title: (source.charAt(0).toUpperCase() + source.slice(1)),
                    collections: JSON.stringify(cList)
                  },
                  silent: true,
                  cleanCollections: false
                });
            });

            component.context.router.transitionTo('uploadBookmarks');
          }else{
            var nextProgress = parseInt((100/this.count)*(this.index+1));
            if (component.state.progress != nextProgress)
              component.setState({progress: nextProgress});
        ...
        next: function() {
          ...
          Api.post('check/collection', {title: _this.items[_this.index].folder}, function(cResult){
  					if (cResult.result)
              _this.check(cResult.id, true);
  					else
              Api.post('collection', {title: _this.items[_this.index].folder/*, count: _this.items[_this.index].items.length*/}, function(aResult){
                _this.check(aResult.item._id, false);
  						});
  				});
        }
      }
      ...
      for(var i in queue.items)
        worker.items.push({folder: i, items: queue.items[i]});
      worker.count = worker.items.length;
...
},{"moment/locale/de.js":"moment/locale/de.js","moment/locale/es.js":"moment/locale/es.js","moment/locale/fi.js":"moment/locale/fi.js","moment/locale/fr.js":"moment/locale/fr.js","moment/locale/id.js":"moment/locale/id.js","moment/locale/ko.js":"moment/locale/ko.js","moment/locale/nl.js":"moment/locale/nl.js","moment/locale/pl.js":"moment/locale/pl.js","moment/locale/pt.js":"moment/locale/pt.js","moment/locale/ru.js":"moment/locale/ru.js","moment/locale/tr.js":"moment/locale/tr.js","moment/locale/uk.js":"moment/locale/uk.js","moment/locale/zh-cn.js":"moment/locale/zh-cn.js","moment/locale/zh-tw.js":"moment/locale/zh-tw.js"}],20:[function(require,module,exports){
...
},{"../actions/Bookmarks":2,"../actions/Childrens":3,"../actions/Collections":4,"../actions/User":7,"../stores/Stats":22}],22:[function(require,module,exports){
...
},{"../actions/Collections":4,"../actions/Stats":6}],23:[function(require,module,exports){
var UserActions = require('../actions/User');
...
},{"../actions/ModalFrame":5,"../actions/User":7,"./Collections":21}]},{},[10]);
